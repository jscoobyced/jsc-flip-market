...
const issueTokens = (user: { id: string; email: string; userType: 'FLIPPER' | 'OWNER' }) => ({
  accessToken: signAccessToken({ sub: user.id, email: user.email, role: user.userType }),
  refreshToken: signRefreshToken({ sub: user.id, email: user.email, role: user.userType }),
});
