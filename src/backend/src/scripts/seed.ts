...
export const seedSampleData = async (): Promise<void> => {
  ...
  const oldProperties = await listProperties({
    page: 1,
    limit: 100,
    ownerId: owner.id,
  });
  for (const prop of oldProperties.items) {
    ...
    await query("DELETE FROM properties WHERE id = $1 AND owner_id = $2", [
      prop.id,
      owner.id,
    ]);
  }
  ...
  for (let i = 1; i <= 3; i++) {
    const propertyImagesForThisProperty = propertyImages.slice(
      (i - 1) * 3,
      i * 3,
    );
    await createProperty(
      {
        ...
        state: "TX",
        zipCode: `7870${i}`,
        propertyType: ["single-family", "multi-family", "commercial"][i - 1] as
          | "single-family"
          | "multi-family"
          | "commercial"
          | "land",
        condition: ["needs-work", "fair", "good", "poor"][i % 4] as
          | "poor"
          | "fair"
          | "needs-work"
          | "good",
