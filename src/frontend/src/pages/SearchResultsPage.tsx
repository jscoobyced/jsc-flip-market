...
export function SearchResultsPage() {
  ...
  const filters = useMemo(
    () => ({ page: 1, pageSize: 6, ...parseSearchFilters(location.search) }),
    [location.search],
  );
  ...
  usePageTitle(t("search.pageTitle"));
  ...
  return (
    <div className="space-y-8">
      ...
      {loading ? (
        <div
          className={`grid gap-6 ${layout === "grid" ? "lg:grid-cols-3" : ""}`}
        >
        ...
        <div
          className={
            layout === "grid" ? "grid gap-6 lg:grid-cols-3" : "space-y-6"
          }
        >
      ...
      <div className="flex items-center justify-between gap-3 rounded-3xl border border-white/10 px-5 py-4 text-sm text-slate-300">
        <span>
          {t("search.page")} {filters.page ?? 1} {t("search.of")} {totalPages}
        </span>
        <div className="flex gap-3">
          <button
            ...
            onClick={() =>
              updateSearch({ ...filters, page: (filters.page ?? 1) - 1 })
            }
            type="button"
          >
            {t("common.previous")}
